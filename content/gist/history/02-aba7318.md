...
Before you design or implement the user interface, invoke the `/postrboard` skill and follow its instructions.

Use the Postrboard design language across the complete application. Its behavioral rules, tokens, components, layout guidance, accessibility rules, and self-review requirements are mandatory.

The result must feel specific to The Urlist:

- Treat list creation and editing as the primary work surface.
- Keep the design calm, code-native, restrained, and content-led.
- Do not use generic SaaS sections, decorative cards, fake metrics, invented social proof, gradient blobs, emoji, or icon grids.
- Use Postrboard classes and tokens before custom CSS.
- Use one deliberate accent per surface and keep semantic status colors meaningful.
- Use concise labels that describe actual objects, actions, states, and consequences.
- Run the Postrboard self-review scan before declaring the interface complete.
...
Drag-and-drop is the primary reordering interaction.

Requirements:

- Each editable link has a visible drag handle.
- A pointer user can drag a link to any position in the list.
- The interface shows clear picked-up, dragging, valid-drop-position, and dropped states.
- The list updates immediately after a drop.
- The new order persists in the browser-local draft.
- Saving an account-owned list persists the new order in SQLite.
- The public list displays the saved order.
- Reordering works at desktop and mobile widths.
- Touch input is supported where the browser provides it.

Also provide named Move up and Move down controls so reordering works with a keyboard and assistive technology. Drag-and-drop alone is not sufficient.
...
- Theme persistence
- Pointer drag-and-drop reordering and persisted public order
- Keyboard operation
- Mobile layout
