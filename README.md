...
- Pages source = **GitHub Actions**, workflow `.github/workflows/deploy.yml`.
  ...
  Playwright suite on every push and the deploy waits for both, about 4 minutes
  in all. Browser failures appear as annotations on the run and the HTML report
  is attached to the run as an artifact for a week. Pushes to other branches run
  the same `build` and `e2e` jobs as CI and skip the deploy job. No secrets:
